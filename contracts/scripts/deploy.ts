...
async function main() {
  ...
  try {
    // Step 1: Deploy MiMC Hasher (Production)
    console.log('Step 1/3: Deploying MiMC-Sponge Hasher (Production)...');
    ...
    console.log('✓ MiMC-Sponge Hasher deployed:', hasherAddress);
    console.log('  (Production implementation with 220 rounds)');
    console.log('');

    // Step 2: Deploy Verifier
    console.log('Step 2/3: Deploying Groth16 Verifier...');
    console.log("NOTE: Make sure you've run 'npm run setup:circuits' first!");
    ...
    fs.writeFileSync(DEPLOYMENT_FILE, JSON.stringify(deploymentAddresses, null, 2));
