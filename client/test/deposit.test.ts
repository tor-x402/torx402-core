...
describe("torx402 Client Library - Deposits", function () {
  ...
  // Initialize cryptographic libraries before all tests
  before(async function () {
    this.timeout(60000); // Initialization can take time
    // Initialize all crypto libraries (Pedersen + MiMC)
  ...
  describe("Note Format", function () {
    ...
    it("Should handle note without leafIndex", async function () {
      ...
      expect(parsed.leafIndex === undefined || parsed.leafIndex === 0).to.be
        .true;
    });
  });
